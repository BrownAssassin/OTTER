...
<<<<<<< HEAD
	...
	// Remove shader parts to save space (we can do this since we only needed the shader parts to compile an actualshader program)
	...
	if (status == GL_FALSE) {
		...
		if (length > 0) {
			...
			LOG_ERROR("Shader failed to link for an unknownreason!");
		}
	}
