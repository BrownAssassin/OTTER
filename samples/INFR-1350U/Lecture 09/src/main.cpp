...
void loadImage() {
	...
	image = stbi_load("cobblestone2.jpg",
		&width,
		&height,
		&channels,
		0);
...
void key_callback(GLFWwindow*, int key, int scancode, int action, int mods) {
	...
	if (key == GLFW_KEY_M && action == GLFW_PRESS) {
		...
		std::cout << "Mipmapping ON" << std::endl;
	}
	...
	if (key == GLFW_KEY_A && action == GLFW_PRESS) {
		...
		std::cout << "Anisotropic Filtering: AF 16" << std::endl;
	}
...
int main() {
	...
	// Cube data
	static const GLfloat points[] = {//floor
		-100.f, -1.0f, 100.0f,
		100.0f, -1.0f, 100.0f,
		-100.0f, -1.0f, -100.0f,
		100.0f, -1.0f, 100.0f,
	...
	glTexParameterf(GL_TEXTURE_2D, GL_TEXTURE_MAX_ANISOTROPY, a);
