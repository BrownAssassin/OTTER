...
int main() {
	...
	interleaved2_ibo->LoadData(indices, 3 * 2);
	...
	while (!glfwWindowShouldClose(window)) {
		...
		transform3 = glm::translate(glm::mat4(1.0f), glm::vec3(0, glm::sin(static_cast<float>(thisFrame)), 0.0f)) * 
					 glm::rotate(glm::mat4(1.0f), static_cast<float>(thisFrame), glm::vec3(0, 0, 1));
		...
		vao3->Bind();
		glDrawElements(GL_TRIANGLES, interleaved2_ibo->GetElementCount(), interleaved2_ibo->GetElementType(), nullptr);
		vao3->UnBind();
