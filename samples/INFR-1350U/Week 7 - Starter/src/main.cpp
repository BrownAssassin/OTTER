...
int main() {
	...
	shader->SetUniform("u_TexMixing", texMixing);
	...
	imGuiCallbacks.push_back([&]() {
		...
		if (ImGui::CollapsingHeader("Material Level Lighting Settings"))
		{
			...
			if (ImGui::SliderFloat("Texture Mixing", &texMixing, 0.0f, 1.0f)) {
				shader->SetUniform("u_TexMixing", texMixing);
			}
		}
	});
	...
	Texture2DData::sptr diffuseMapTwo = Texture2DData::LoadFromFile("images/MetalPlate_Diffuse.png");
	...
	Texture2D::sptr diffuseTwo = Texture2D::Create();
	diffuseTwo->LoadData(diffuseMapTwo);
	...
	while (!glfwWindowShouldClose(window)) {
		...
		// Tell OpenGL that slot 0 will hold the diffuse, slot 1 will hold the second diffuse, and slot 2 will hold the specular
		shader->SetUniform("s_Diffuse",  0);
		shader->SetUniform("s_DiffuseTwo", 1);
		...
		for(int ix = 0; ix < 4; ix++) {
			...
			materials[ix].AlbedoTwo->Bind(1);
			materials[ix].Specular->Bind(2);
			shader->SetUniform("u_Shininess", materials[ix].Shininess);
			shader->SetUniform("u_TexMixing", materials[ix].TexMixing);
