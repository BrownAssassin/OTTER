...
uniform sampler2D s_DiffuseTwo;
...
void main() {
	...
	vec4 textureColorTwo = texture(s_DiffuseTwo, inUV);
	vec4 textureResult = (1.0 - u_TexMixing) * textureColor + u_TexMixing * textureColorTwo;
	vec3 result = (
		(u_AmbientCol * u_AmbientStrength) + // global ambient light
		(ambient + diffuse + specular) * attenuation // light factors from our single light
		) * inColor * textureResult.rgb; // Object color

	frag_color = vec4(result, textureResult.a);
}